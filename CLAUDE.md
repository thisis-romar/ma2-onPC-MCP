...
isolation (transcript §1.1) requires an LLM client (Claude API / Agent SDK). The
`sub_agent_fn` parameter in `Orchestrator.__init__` is the correct injection point for
integrators who want to wire in a real subagent spawner.

**LTM session compression** — `LongTermMemory.save_session()` stores a compressed
decision summary (`_v: 2` format) rather than the full `wm.to_dict()` blob. Fixture
detail is already in the `fixture_history` table; the snapshot retains only decisions:
`completed_steps`, `failed_steps`, `token_spend`, `park_ledger`, `console_state_summary`,
and `fixture_summary` (names + values only). `recall_session()` handles both v1 (legacy
full blob) and v2 formats transparently.
...
a token it uses keyword overlap and adds a `warning` field to the response. Pass
`prefer_semantic=False` to force keyword matching regardless of environment.
