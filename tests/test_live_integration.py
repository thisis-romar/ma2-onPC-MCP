...
# All tool functions are imported from the server module.
# After the submodule split, PROFESSIONAL/ENTERPRISE tools may not be
# available (src/private/ is a separate git submodule). Guard the import
# so the test module can at least be collected — individual tests that use
# missing tools will fail at call time, not at import time.
try:
    ...
    pytest.skip(
        f"Live integration tests require src/private/ submodule: {_import_err}",
