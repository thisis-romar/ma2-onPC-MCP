...
An MCP server that exposes **137 tools** so AI assistants can control a grandMA2 lighting console
...
| MCP integration | ✅ Excellent | 137 tools (110 in `server.py` + 27 in `server_orchestration_tools.py`), stdio transport, Claude Desktop + VS Code configs |
...
141-keyword vocabulary classifier (`src/vocab.py`) back this up. OpenSpace mentions none of
this. For a system controlling live physical hardware, this safety model is more mature.

### 3.2 MCP tool quality

137 tools across 12 categories, pure-function command builders in `src/commands/`,
