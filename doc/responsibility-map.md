...
Every module in ma2-onPC-MCP has exactly one primary role. This map enforces the central rule:
...
| `src/commands/helpers.py` | Tool | ✅ Pure helpers (`quote_name`, `_build_store_options`). No smells. |
...
| `src/agent_memory.py` | Memory / State | ✅ WorkingMemory + LongTermMemory + DecisionCheckpoint. ⚠ No progressive disclosure for checkpoints yet |
| `src/console_state.py` | Memory / State | ✅ ConsoleStateSnapshot hydrates 19 show-memory gaps. ⚠ Some hydrations could be lazy-computed |
...
| `src/skill.py` | Skill / Instruction module | ✅ Versioned playbook registry. `as_user_message()` provides injection payload. ⚠ No file-reference / progressive disclosure yet |
...
| `CLAUDE.md` | Skill / Instruction module | ✅ Thin root index. Detail in `.claude/rules/`. |
...
**Location:** `src/orchestrator.py` — `_default_sub_agent()`
...
### S2 — Planner sees all 143 tools (Medium)
**Location:** `src/server.py` — all tools always registered at startup
**Problem:** All 143 tools consume instruction budget in every session.
**Fix:** Use `suggest_tool_for_task` for pre-session retrieval. See `doc/tool-surface-tiers.md`.

### S3 — TaskDecomposer has 3 hardcoded rules (Low)
**Location:** `src/task_decomposer.py`
**Problem:** Only wash_look, blackout_sequence, group_preset_library + generic fallback.
**Fix:** Add more rules as additional MA2 workflows are validated.

### S4 — No progressive disclosure in skills (Low)
**Location:** `src/skill.py` — `body` stored inline
**Problem:** No file-reference mechanism; large playbooks fully loaded even when partial.
**Fix:** Add `references: list[str]` field to Skill dataclass. Load referenced files lazily.

### S5 — server.py instructions= block may be stale (Low)
**Location:** `src/server.py` FastMCP `instructions=` argument
**Problem:** Static tool list may not reflect all 143 current tools.
**Fix:** Replace with a compact summary pointing to `suggest_tool_for_task` for discovery.
