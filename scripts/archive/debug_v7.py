"""Debug v7 macro execution — capture all output and errors, audit fixture order."""
import asyncio
import re
import time
import sys
...
async def main():
    ...
    # Check if macro 7 exists, import if needed
    await go_root(client)
    resp = await client.send_command_with_response("list macro 7", timeout=5.0, delay=1.0)
    c = clean(resp)
    print(f"list macro 7 response:\n{c}\n")

    if "Auto Create Multi-Pools v7" not in c:
        print("Macro 7 not found - importing...")
        resp = await client.send_command_with_response(
            'import "auto_create_multi_pools_v7" at macro 7 /nc',
        ...
        resp = await client.send_command_with_response("list macro 7", timeout=5.0, delay=1.0)
        print(f"After import, list macro 7:\n{clean(resp)}\n")

    # Run macro
    await flush(client)
    print(f"Go Macro 7 -- {time.strftime('%H:%M:%S')}")
    client._writer.write("Go Macro 7\r\n")
